...
On the first launch the app will automatically register itself in the register so that the extension can communicate with it.
To do the communication between the extension and app a .exe written in c++ has been made. This app read the stdin, retrieve the url, put it on a .txt file and launch the app if it's not already launch.
